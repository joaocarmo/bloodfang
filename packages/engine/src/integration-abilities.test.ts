...
function setupControlledGame(p0Hand: CardId[], p1Hand: CardId[], seed = 100): GameState {
  const defs = allDefs();
...
describe('integration: whenPlayed abilities', () => {
  it('enhancer-on-play buffs target at range when played', () => {
    let state = setupControlledGame([testCardId('enhancer-on-play')], [testCardId('filler-1')]);

    // Inject a target card at (0,1) for P0
    const injected = injectCard(state, testCardId('r1-basic'), { row: 0, col: 1 }, 0);
    ...
    state = playCard(state, testCardId('enhancer-on-play'), { row: 0, col: 0 });

    // Target should have +2 bonus power
    const target = defined(state.cardInstances[targetId]);
  ...
  it('enfeeble-on-play debuffs enemy card and triggers death', () => {
    let state = setupControlledGame([testCardId('enfeeble-on-play')], [testCardId('filler-1')]);

    // Inject enemy card at (0,1) with power 2
    const injected = injectCard(state, testCardId('r1-basic'), { row: 0, col: 1 }, 1);
    state = injected.state;
    const targetId = injected.instanceId;

    // P0 plays enfeeble-on-play at (0,0) — enfeebles (0,1) by -3
    state = playCard(state, testCardId('enfeeble-on-play'), { row: 0, col: 0 });
  ...
  it('destroyer-on-play directly destroys target card', () => {
    let state = setupControlledGame([testCardId('destroyer-on-play')], [testCardId('filler-1')]);

    const injected = injectCard(state, testCardId('r1-basic'), { row: 0, col: 1 }, 1);
    state = injected.state;
    const targetId = injected.instanceId;

    state = playCard(state, testCardId('destroyer-on-play'), { row: 0, col: 0 });

    expect(state.cardInstances[targetId]).toBeUndefined();
    expect(cell(state.board, 0, 1).cardInstanceId).toBeNull();
  });

  it('hand-generator adds token cards to hand', () => {
    let state = setupControlledGame([testCardId('hand-generator')], [testCardId('filler-1')]);

    state = playCard(state, testCardId('hand-generator'), { row: 0, col: 0 });
    ...
    expect(state.players[0].hand.filter((id) => id === testCardId('token-basic'))).toHaveLength(2);
  });

  it('spawner creates token cards on board', () => {
    let state = setupControlledGame([testCardId('spawner')], [testCardId('filler-1')]);
    ...
    state = playCard(state, testCardId('spawner'), { row: 0, col: 0 });

    // Token should be spawned at (0,1) — ability range col+1
    expect(cell(state.board, 0, 1).cardInstanceId).not.toBeNull();
    const spawnedId = defined(cell(state.board, 0, 1).cardInstanceId);
    expect(defined(state.cardInstances[spawnedId]).definitionId).toBe('token-basic');
  });

  it('rank-booster adds pawns to target positions', () => {
    let state = setupControlledGame([testCardId('rank-booster')], [testCardId('filler-1')]);

    const pawnsBefore = cell(state.board, 0, 1).pawnCount;
    // Need to ensure (0,1) exists — it's a middle column
    state = playCard(state, testCardId('rank-booster'), { row: 0, col: 0 });

    // Ability range col+1 -> (0,1) gets +2 pawns
    // If it was unowned (pawnCount 0), now it should have 2 (and be owned by P0)
    expect(cell(state.board, 0, 1).pawnCount).toBeGreaterThan(pawnsBefore);
  });
});

describe('integration: whenDestroyed abilities', () => {
  it('death-curse enfeebles enemies in lane when destroyed', () => {
    let state = setupControlledGame([testCardId('filler-1')], [testCardId('filler-1')]);

    // Set up: death-curse (P0) and enemy card in same lane
    const dc = injectCard(state, testCardId('death-curse'), { row: 0, col: 0 }, 0);
    state = dc.state;
    const curseId = dc.instanceId;

    const enemy = injectCard(state, testCardId('r2-basic'), { row: 0, col: 4 }, 1);
    state = enemy.state;
    ...
    expect(defined(state.cardInstances[enemyId]).bonusPower).toBe(-2);
  });
});
...
describe('integration: reaction triggers', () => {
  it('avenger gains +2 when allied card is destroyed', () => {
    ...
    const a = injectCard(state, testCardId('avenger'), { row: 0, col: 0 }, 0);
    state = a.state;
    const avengerId = a.instanceId;

    const ally = injectCard(state, testCardId('r1-basic'), { row: 0, col: 1 }, 0);
    state = ally.state;
    ...
    expect(defined(state.cardInstances[avengerId]).bonusPower).toBe(2);
  });

  it('predator gains +3 when enemy card is destroyed', () => {
    let state = setupControlledGame([testCardId('filler-1')], [testCardId('filler-1')]);

    const p = injectCard(state, testCardId('predator'), { row: 0, col: 0 }, 0);
    state = p.state;
    const predatorId = p.instanceId;

    const enemy = injectCard(state, testCardId('r1-basic'), { row: 0, col: 4 }, 1);
    state = enemy.state;
    const enemyId = enemy.instanceId;

    state = destroyCard(state, enemyId);

    expect(defined(state.cardInstances[predatorId]).bonusPower).toBe(3);
  });

  it('scavenger gains +1 when any non-self card is destroyed', () => {
    let state = setupControlledGame([testCardId('filler-1')], [testCardId('filler-1')]);

    const s = injectCard(state, testCardId('scavenger'), { row: 0, col: 0 }, 0);
    state = s.state;
    const scavengerId = s.instanceId;

    const victim = injectCard(state, testCardId('r1-basic'), { row: 1, col: 4 }, 1);
    state = victim.state;

    state = destroyCard(state, victim.instanceId);

    expect(defined(state.cardInstances[scavengerId]).bonusPower).toBe(1);
  });
});
...
describe('integration: continuous effects', () => {
  it('aura-buffer gives +1 to adjacent cards while alive', () => {
    ...
    const aura = injectCard(state, testCardId('aura-buffer'), { row: 0, col: 2 }, 0);
    state = aura.state;

    const left = injectCard(state, testCardId('r1-basic'), { row: 0, col: 1 }, 0);
    state = left.state;
    const leftId = left.instanceId;

    const right = injectCard(state, testCardId('r1-cross'), { row: 0, col: 3 }, 0);
    state = right.state;
  ...
  it('aura-buffer effect disappears when destroyed', () => {
    ...
    const target = injectCard(state, testCardId('r1-basic'), { row: 0, col: 3 }, 0);
    state = target.state;
  ...
  it('army-scaler power increases with allied cards in lane', () => {
    ...
    const scaler = injectCard(state, testCardId('army-scaler'), { row: 0, col: 0 }, 0);
    state = scaler.state;
    ...
    const ally1 = injectCard(state, testCardId('r1-basic'), { row: 0, col: 1 }, 0);
    state = ally1.state;
    ...
    // Add another ally
    const ally2 = injectCard(state, testCardId('r1-cross'), { row: 0, col: 2 }, 0);
    state = ally2.state;
  ...
  it('dual-aura buffs allies and debuffs enemies', () => {
    ...
    const aura = injectCard(state, testCardId('dual-aura'), { row: 0, col: 2 }, 0);
    state = aura.state;
    ...
    const enemy = injectCard(state, testCardId('r1-basic'), { row: 0, col: 3 }, 1);
    state = enemy.state;
    const enemyId = enemy.instanceId;
...
describe('integration: cascade scenarios', () => {
  it('enfeeble -> death -> whenDestroyed cascade', () => {
    ...
    let state = setupControlledGame([testCardId('chain-killer')], [testCardId('filler-1')]);

    // Set up death-curse as the target
    const curse = injectCard(state, testCardId('death-curse'), { row: 0, col: 1 }, 1);
    state = curse.state;
    const curseId = curse.instanceId;

    // And a P0 card in the same lane that death-curse will target
    const p0card = injectCard(state, testCardId('r2-basic'), { row: 0, col: 2 }, 0);
    state = p0card.state;

    // chain-killer at (0,0) targets (0,1) via ability range col+1
    state = playCard(state, testCardId('chain-killer'), { row: 0, col: 0 });
    ...
    const killerCardId = cell(state.board, 0, 0).cardInstanceId;
  ...
  it('cascadeGrower gains power from chain deaths', () => {
    ...
    const target = injectCard(state, testCardId('r1-basic'), { row: 0, col: 1 }, 1);
    state = target.state;

    // Cascade grower watches for any destruction
    const grower = injectCard(state, testCardId('cascade-grower'), { row: 1, col: 0 }, 0);
    state = grower.state;
    ...
    expect(defined(state.cardInstances[growerId]).bonusPower).toBe(2);
  });
  ...
  it('multiple death reactions fire in order', () => {
    ...
    const s1 = injectCard(state, testCardId('scavenger'), { row: 0, col: 0 }, 0);
    state = s1.state;
    const s1Id = s1.instanceId;

    const s2 = injectCard(state, testCardId('cascade-grower'), { row: 1, col: 0 }, 0);
    state = s2.state;
    const s2Id = s2.instanceId;

    // Destroy a third card
    const victim = injectCard(state, testCardId('r1-basic'), { row: 2, col: 4 }, 1);
    state = victim.state;

    state = destroyCard(state, victim.instanceId);

    // Both should have gained bonuses
    expect(defined(state.cardInstances[s1Id]).bonusPower).toBe(1); // scavenger +1
    expect(defined(state.cardInstances[s2Id]).bonusPower).toBe(2); // cascade-grower +2
  });
});
...
describe('integration: one-shot flag triggers', () => {
  it('whenFirstEnfeebled fires once then never again', () => {
    let state = setupControlledGame(
      [testCardId('enfeeble-on-play'), testCardId('enfeeble-on-play')],
      [testCardId('filler-1')],
    );

    // Set up resilient card as target
    const res = injectCard(state, testCardId('resilient'), { row: 0, col: 1 }, 1);
    state = res.state;
    ...
    expect(defined(resilientAfter).bonusPower).toBe(0); // -3 + 3
    expect(defined(resilientAfter).hasBeenEnfeebled).toBe(true);
  });
  ...
  it('whenPowerReachesN fires once when threshold met', () => {
    ...
    const td = injectCard(state, testCardId('threshold5-destroyer'), { row: 0, col: 1 }, 0);
    state = td.state;
    ...
    expect(defined(state.cardInstances[tdId]).powerReachedN).toBe(true);
...
describe('integration: endOfGame scoring', () => {
  it('laneScoreBonus adds bonus points to lane score', () => {
    ...
    const lb = injectCard(state, testCardId('lane-bonus'), { row: 0, col: 0 }, 0);
    state = lb.state;
  ...
  it('scoreRedistribution gives lane winner both scores', () => {
    ...
    // P0 card in lane 0
    const p0card = injectCard(state, testCardId('r2-basic'), { row: 0, col: 0 }, 0); // power 5
    state = p0card.state;

    // P1 card in lane 0
    const p1card = injectCard(state, testCardId('r1-basic'), { row: 0, col: 4 }, 1); // power 2
    state = p1card.state;

    // Score redistributor in lane 0
    const sr = injectCard(state, testCardId('score-redistributor'), { row: 0, col: 2 }, 0);
    state = sr.state;
...
describe('integration: continuous effect lifecycle', () => {
  it('whileInPlay modifiers are rebuilt after each ability resolution', () => {
    let state = setupControlledGame([testCardId('aura-buffer')], [testCardId('filler-1')]);

    // Place a target card adjacent to where aura will be placed
    const target = injectCard(state, testCardId('r1-basic'), { row: 0, col: 1 }, 0);
    state = target.state;
    const targetId = target.instanceId;
    ...
    state = playCard(state, testCardId('aura-buffer'), { row: 0, col: 0 });
...
describe('integration: range mirroring for abilities', () => {
  it('ability range mirrors for player 1', () => {
    let state = setupControlledGame([testCardId('filler-1')], [testCardId('enhancer-on-play')]);
    ...
    // Target at (0,3)
    const target = injectCard(state, testCardId('r1-basic'), { row: 0, col: 3 }, 1);
    state = target.state;
    const targetId = target.instanceId;

    state = playCard(state, testCardId('enhancer-on-play'), { row: 0, col: 4 });

    expect(defined(state.cardInstances[targetId]).bonusPower).toBe(2);
  });
});
...
describe('integration: full game flow with abilities', () => {
  it('complete game with ability cards from start to scoring', () => {
    let state = setupControlledGame(
      [testCardId('enhancer-on-play'), testCardId('r1-basic'), testCardId('lane-bonus')],
      [testCardId('r1-basic'), testCardId('r1-cross')],
    );

    // P0: play r1-basic at (0,0) — setup for enhancement
    state = playCard(state, testCardId('r1-basic'), { row: 0, col: 0 });

    // P1: play r1-basic at (0,4)
    state = playCard(state, testCardId('r1-basic'), { row: 0, col: 4 });

    // P0: need to play on a valid tile. Col 0 row 1 is still P0's
    // Play lane-bonus at (1,0)
    state = playCard(state, testCardId('lane-bonus'), { row: 1, col: 0 });
...
describe('integration: token card support', () => {
  it('token definitions are accepted in card definitions', () => {
    ...
    expect(defined(defs['token-basic']).isToken).toBe(true);
  });
  ...
  it('spawned token cards have correct properties', () => {
    ...
    const spawnedId = cell(state.board, 0, 1).cardInstanceId;
    expect(spawnedId).not.toBeNull();
    const spawned = defined(state.cardInstances[defined(spawnedId)]);
...
describe('integration: whenAlliedPlayed / whenEnemyPlayed triggers', () => {
  it('ally-watcher gains +1 when owner plays another card', () => {
    let state = setupControlledGame([testCardId('r1-basic')], [testCardId('filler-1')]);

    // Inject ally-watcher already on board for P0
    const watcher = injectCard(state, testCardId('ally-watcher'), { row: 1, col: 0 }, 0);
    state = watcher.state;
    ...
    expect(defined(state.cardInstances[watcherId]).bonusPower).toBe(1);
  });

  it('ally-watcher does NOT trigger for enemy plays', () => {
    let state = setupControlledGame([testCardId('filler-1')], [testCardId('r1-basic')]);
    ...
    expect(defined(state.cardInstances[watcherId]).bonusPower).toBe(0);
  });

  it('ally-watcher does NOT trigger for its own play', () => {
    // If ally-watcher is in hand and played, it should not trigger itself
    let state = setupControlledGame([testCardId('ally-watcher')], [testCardId('filler-1')]);

    state = playCard(state, testCardId('ally-watcher'), { row: 0, col: 0 });

    // Find the ally-watcher instance on the board
    const tile = cell(state.board, 0, 0);
    const instanceId = defined(tile.cardInstanceId);
    expect(defined(state.cardInstances[instanceId]).bonusPower).toBe(0);
  });
  ...
  it('enemy-watcher gains +2 when opponent plays a card', () => {
    ...
    const watcher = injectCard(state, testCardId('enemy-watcher'), { row: 1, col: 0 }, 0);
    state = watcher.state;
    const watcherId = watcher.instanceId;
    ...
    expect(defined(state.cardInstances[watcherId]).bonusPower).toBe(2);
  });
...
describe('integration: filtered target selectors (enhanced/enfeebled)', () => {
  it('enfeebled-hunter targets only already-enfeebled enemy cards', () => {
    let state = setupControlledGame(
      [testCardId('enfeeble-on-play'), testCardId('enfeebled-hunter')],
      [testCardId('filler-1')],
    );

    // Inject enemy at (0,1) to be in enfeeble-on-play's ability range (col+1)
    const e1 = injectCard(state, testCardId('r2-basic'), { row: 0, col: 1 }, 1);
    state = e1.state;
    const e1Id = e1.instanceId;

    // Inject another enemy NOT in ability range
    const e2 = injectCard(state, testCardId('r2-basic'), { row: 1, col: 4 }, 1);
    state = e2.state;
    ...
    expect(defined(state.cardInstances[e1Id]).hasBeenEnfeebled).toBe(true);
    expect(defined(state.cardInstances[e2Id]).hasBeenEnfeebled).toBeUndefined();
    ...
    state = playCard(state, testCardId('enfeebled-hunter'), { row: 1, col: 0 });
    ...
    expect(defined(state.cardInstances[e2Id]).bonusPower).toBe(0);
  });

  it('enhanced-booster targets only already-enhanced allied cards', () => {
    let state = setupControlledGame(
      [testCardId('enhancer-on-play'), testCardId('enhanced-booster')],
      [testCardId('filler-1')],
    );
    ...
    const ally2 = injectCard(state, testCardId('r1-basic'), { row: 1, col: 1 }, 0);
    state = ally2.state;
    ...
    expect(defined(state.cardInstances[ally1Id]).hasBeenEnhanced).toBe(true);
    expect(defined(state.cardInstances[ally2Id]).hasBeenEnhanced).toBeUndefined();
    ...
    state = playCard(state, testCardId('enhanced-booster'), { row: 1, col: 0 });

    expect(defined(state.cardInstances[ally1Id]).bonusPower).toBe(4); // +2 from enhancer + +2 from booster
    expect(defined(state.cardInstances[ally2Id]).bonusPower).toBe(0); // untouched
  });
});
...
describe('integration: dynamicValue replacement effects', () => {
  it('replacement-dynamic gains power equal to replaced card effective power', () => {
    let state = setupControlledGame([testCardId('replacement-dynamic')], [testCardId('filler-1')]);

    // Inject an allied card to replace — r2-basic has power 5
    const target = injectCard(state, testCardId('r2-basic'), { row: 0, col: 0 }, 0);
    state = target.state;
    const targetId = target.instanceId;

    // P0 plays replacement-dynamic at (0,0) replacing r2-basic
    state = playCard(state, testCardId('replacement-dynamic'), { row: 0, col: 0 });

    // Old card should be destroyed
    expect(state.cardInstances[targetId]).toBeUndefined();

    // Find the replacement card instance
    const repInstanceId = cell(state.board, 0, 0).cardInstanceId;
    expect(repInstanceId).not.toBeNull();
    const repInstance = defined(state.cardInstances[defined(repInstanceId)]);
    ...
    expect(getEffectivePower(state, defined(repInstanceId))).toBe(6);
  });
  ...
  it('replacement-dynamic accounts for modifiers on replaced card', () => {
    ...
    const target = injectCard(state, testCardId('r1-basic'), { row: 0, col: 0 }, 0);
    state = target.state;
    const targetId = target.instanceId;
    ...
    const repInstanceId = defined(cell(state.board, 0, 0).cardInstanceId);
...
describe('integration: multi-token addCardToHand', () => {
  it('multi-token-generator adds different token types to hand', () => {
    let state = setupControlledGame(
      [testCardId('multi-token-generator')],
      [testCardId('filler-1')],
    );

    state = playCard(state, testCardId('multi-token-generator'), { row: 0, col: 0 });

    // P0 hand should have 1 token-basic + 1 token-strong
    const p0Hand = state.players[0].hand;
    expect(p0Hand.filter((id) => id === testCardId('token-basic'))).toHaveLength(1);
    expect(p0Hand.filter((id) => id === testCardId('token-strong'))).toHaveLength(1);
  });
});
